...
public class UserServiceImpl implements UserService{
	...
	@Override
	public List<FIR> getAllFIRsFiledByUser(Integer userId,String key) throws UserException, FIRException, LoginException {
		...
		if(opt.isPresent()){
			...
			if(cus.getContact().equals(opt.get().getMobileNumber())){
				User user=opt.get();
				if(user.getFirs().size() == 0) throw new FIRException("No FIRs filed by this user");
			
				return user.getFirs();
			}else{
				throw new LoginException("User ID and Login Key doesn't match.");
			}
...
@Override
public String withdraw_FIR(Integer userID,String password,String key, Integer firId) throws UserException, FIRException, LoginException {
